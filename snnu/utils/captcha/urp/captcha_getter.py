...
#         print(proxies)
        try:
            r = requests.get(url_captcha, proxies=proxies, headers=headers, timeout=3).content
        except:
            err += 1
            print("ERROR:{}/{} {}% ip:{}".format(err, pro, round(err * 1.0 / pro * 100, 2), ip))
            continue
        md5 = Hash(r)
        if md5 not in save_image:
            ...
            rep += 1
            print("Existed:{}/{}\t{}% {}%".format(rep, pro, round(rep * 1.0 / pro * 100, 2), round(rep * 1.0 / (pro - err) * 100, 2)))
...
def GetIpList():
    ...
    table = soup.find(name='table', attrs={"class":"table table-hover table-bordered table-striped"})
